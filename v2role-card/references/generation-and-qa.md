...
- 只把背景整理为干净的白色或近白中性角色档案背景，不把角色本身转换为另一种媒介。
...
Asset type: 4:3 landscape character reference sheet for production, rendered in the confirmed source-video style
...
Confirmed source-video style: [confirmed medium/rendering method; character proportions and shape language; line/edge treatment; shading, materials and texture; color, contrast and temperature; camera/lighting language]. Match these observable style properties faithfully in all six views. Do not convert the character to photorealism or any other medium unless that is the confirmed source-video style.
Scene/backdrop: clean white or near-white neutral character-dossier background rendered compatibly with the confirmed style, with only the minimal grounding cue appropriate to that style.
...
Pose/expression: neutral relaxed standing pose for full-body views, arms naturally at sides; [confirmed neutral expression] in face close-ups; accurate anatomy or character-design structure for the confirmed style.
...
Lighting/mood: [confirmed light/shadow treatment] applied evenly enough for a readable reference sheet without changing the source style.
Constraints: exactly one isolated target character per panel and exactly six panels; the same person, same outfit and same confirmed source-video style in every panel; infer unseen side and back details conservatively from confirmed information; no other people; no props; no source-video scene; no extra limbs or broken character structure; no duplicated figures within a panel; no text, labels, numbers, arrows, logos, borders, watermarks, subtitles, interface elements or decorative graphics.
```
...
- 六个画面的媒介、人物比例、造型语言、线条或边缘、色彩、明暗、材质、纹理和镜头/光照与用户确认的视频风格一致；
- 白色或近白中性背景干净，并以与角色相同的媒介呈现；
...
```text
Change only [one problem]. Keep the same character identity, apparent age, facial proportions, hairstyle, body proportions, wardrobe, confirmed source-video style, six-panel layout, neutral white background and all other views unchanged. [Precise correction]. No text, logos or watermark.
```
