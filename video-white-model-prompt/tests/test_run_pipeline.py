...
class ResumePipelineTests(unittest.TestCase):
    ...
    def test_resume_reuses_draft_and_depth_cache(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def fake_encode(command: list[str]) -> int:
            ...
            with (
                ...
                mock.patch.object(MODULE, "run_process", side_effect=fake_encode),
            ...
            self.assertTrue((args.output_dir / "completed.json").is_file())
            self.assertTrue(
                (
                    args.output_dir / "depth_previous_1/input_depth_720p_part_01.mp4"
                ).is_file()
            )
            self.assertTrue(
                (args.output_dir / "depth/input_depth_720p_part_01.mp4").is_file()
